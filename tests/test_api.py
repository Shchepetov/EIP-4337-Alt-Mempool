...
@pytest.mark.eth_sendUserOperation
@pytest.mark.asyncio
async def test_sent_user_op_saved(client: AsyncClient, test_request: dict):
    user_op_hash = (
        await client.post("/api/eth_sendUserOperation", json=test_request)
    ).json()
    user_op = (
        await client.post(
            "/api/eth_getUserOperationByHash", json={"hash": user_op_hash}
        )
    ).json()
